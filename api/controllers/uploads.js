...
export default {
  ...
  async afterUpload(
    ...
    if ((typesFile[type] === 9 || typesFile[type] === 10) && !conferenceId)
      throw new AppErrorMissing("conferenceId");
    ...
    if (reportId) {
      ...
      typesFile[type] === 4
        ? await report.update({ reportFile: file.path })
        : await report.update({ conclusion: file.path });
      return res.json({ url: file.path });
    }
    ...
    if (typesFile[type] === 9 || typesFile[type] === 10) {
      ...
      if (typesFile[type] === 9)
        await participantInConference.update({ accord: file.path });
