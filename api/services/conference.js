...
async function processJsonField(
  conference,
  conferenceInfo,
  fieldName,
  types,
...
async function processArrayField(conference, conferenceInfo, fieldName) {
  if (conference?.[fieldName] && conferenceInfo?.[fieldName]) {
    ...
    const updatedFiles = conference[fieldName].filter(
      (file) => !filesToKeep.has(file)
    );
...
export default {
  ...
  async update(conferenceInfo, conferenceId) {
    ...
    if (conferenceInfo.deadline) {
      if (
        ...
        conferenceInfo?.stages?.length > 0
      ) {
        const isDeadlineInStages = conference.stages?.some(
          (item) => item.date === conferenceInfo.deadline
        );
        const isDeadlineInConferenceInfoStages = conferenceInfo.stages?.some(
          (item) => item.date === conferenceInfo.deadline
        );
  ...
  async assignFee(feeInfo) {
    ...
    if (ids.length !== participantInConference.length)
      AppErrorNotExist("participantInConference");
    ...
    return await ParticipantInConference.bulkCreate(feeInfo, {
  ...
  async exportReports(conferenceId) {
    const reports = await Report.findAll({
      ...
      group: [
        ...
        "Report.name",
        "Report.id",
        "participantOfReport.id",
        "participantOfReport.participant.id",
      ], // Grouping corrected
    });
    ...
    reports.forEach((report) => {
      ...
      const rows = report.participantOfReport.map((data) => [
        data.who,
        `${data.participant.surname} ${data.participant.name} ${
