...
export default {
  async create(reportInfo, conferenceId, participant) {
    ...
    if (reportInfo?.coAuthors?.length > 0) {
      ...
      const participantsExistIds = participantsExist.map(
        (participantExist) => participantExist.id
      );
      ...
      if (participantsExistIds.length > 0) {
        ...
        const participantInConferencePromises = participantsExistIds.map(
          (participantId) => {
      ...
      if (participantsExist?.length > 0) {
        const existingEmails = new Set(
          participantsExist.map((participant) => participant.email)
        );
        emailsNotInDatabase = emails.filter(
      ...
      emailsNotInDatabase.forEach((email) => {
        const person = reportInfo.coAuthors.find(
  ...
  async updateDirections(reportsInfo) {
    ...
    if (reports.length !== reportsIds.length)
      throw new AppErrorInvalid("reportsIds");
    ...
    return await Report.bulkCreate(reportsInfo, {
  ...
  async findOne(reportId, participant) {
    ...
    report.cacheCoAuthors = Object.values(cache)
      ?.flat()
      ?.filter((a) => a === report.id)?.length;
