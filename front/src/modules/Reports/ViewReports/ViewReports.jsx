    ...
    return ( 
        <section className={styles.ViewReports}>
            <div className={styles.ViewReportsInner}>
                <div className={styles.ViewReportsInnerFirst}>
                    <div className={styles.ViewReportsBlock}>
                        <p className={styles.ViewReportsTitle}>Название доклада № {reportData?.number}:</p>
                        <p>{reportData?.name}</p>
                    </div>
                    <div className={styles.ViewReportsBlock}>
                ...
                <div className={styles.ViewReportsInnerSecond}>
                    <div className={styles.ViewReportsSoauthors}>
                        ...
                        {reportData?.soauthors.map((item, index) => (
                            <div key={index}>
                                <p className={styles.name}>{`${index + 1}. ${item?.name} ${item?.surname} ${item?.patronymic}`}</p>
                                <ul>
                                    <li> <p>{item?.organization}</p></li>
            ...
            <div className={styles.fileLouders}>
                <div className={styles.fileContur} onMouseEnter={() => handleMouseEnter(1)} onMouseLeave={() => handleMouseLeave()} onMouseMove={handleMouseMove}>
                ...
                <div className={styles.fileContur} onMouseEnter={() => handleMouseEnter(2)} onMouseLeave={() => handleMouseLeave()} onMouseMove={handleMouseMove}>
            ...
            <div className={styles.EditDataReport}>
                <button onClick={() => navigate(`/Lks/editReport?idReport=${reportData?.id}`)}>Редактировать данные</button>
            </div>
        </section>
    );
}
